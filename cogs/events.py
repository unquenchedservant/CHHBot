...
class Events(commands.Cog):
    def __init__(self, bot):
        ...
        self.va_release_db = VAReleasesDB()
    ...
    async def archive_check(self):
        ...
        #we're going to hijack this for verified artist releases
        current_date = datetime.now().strftime('%m/%d/%y')
        releases = self.va_release_db.get_by_date(current_date)
        if not len(releases) == 0:
            embed = discord.Embed(title="New Release(s) Today!")
            for release in releases:
                _, userID, _, title, date, desc, type_, link = release
                link_text = f"[Link]({link})" if link != "None" else "None"
                
                field_value = f"**Artist:** <@{userID}>\n**Type:** {type_}\n**Description:** {desc}\n**Link:** {link_text}"
    ...
    async def on_message(self, message):
        ...
        if self.bot.user.mentioned_in(message) and not message.author == self.bot.user:
            ...
            async with message.channel.typing():
                response = await openai.generate_answer(message.content, uname, message.channel, self.bot, ref_msg)
