...
async function handleCheck(interaction) {
  ...
  if (!vaReleasesDB.checkLen(releases)) {
    ...
    if (await checkVerified(interaction.user, interaction.guild) && isSelf) {
    ...
    for (const release of releases) {
      ...
      const fieldValue = `**Release Date:** ${date}\n**Type:** ${rType}\n**Description:** ${desc}\n**Link:** ${linkText}`;
      const fieldName = (await checkVerified(interaction.user, interaction.guild) && isSelf) ? `ID: #${releaseID} - ${title}` : `${title}`;
