...
async function addHoliday(month, day, msg) {
  const updated = await holidayDB.add(month, day, msg);
...
async function removeHoliday(month, day) {
  const status = holidayDB.remove(month, day);
...
async function checkHoliday(interaction) {
  let responseMsg = '';
  const month = interaction.options.getInteger('month');
  const day = interaction.options.getInteger('day');
  if (month === null && day === null) {
    const holidays = await holidayDB.checkMulti();
    ...
    const mesg = await holidayDB.check(month, day);
    if (mesg == 0) {
...
module.exports = {
  ...
  async execute(interaction) {
    if (interaction.options.getSubcommand() === 'add') {
      ...
      const response = await addHoliday(interaction.options.getInteger('month'), interaction.options.getInteger('day'), interaction.options.getString('message'));
      await interaction.reply({ content: response, flags: MessageFlags.Ephemeral });
    }
    else if (interaction.options.getSubcommand() === 'remove') {
      logger.info(`'/holiday remove' was called by ${interaction.user.tag}`);
      const response = await removeHoliday(interaction.options.getInteger('month'), interaction.options.getInteger('day'));
      await interaction.reply({ content: response, flags: MessageFlags.Ephemeral });
    }
    else if (interaction.options.getSubcommand() === 'check') {
      logger.info(`'/holiday check' was called by ${interaction.user.tag}`);
      await checkHoliday(interaction);
    }
  },
};