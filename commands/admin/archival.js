...
async function handle_existing_archive(channel, level, check){
    if (check[0][3] == 2 && level == 1){
        ...
        await archival_db.update(channel, level=level, month=current_month)
    }else{
        await archival_db.update(channel, level=level)
    }
}
